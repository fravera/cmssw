...
void CTPPSPixelLocalTrackProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup){
  ...
  for(const auto & pattern : patternVector){
    ...
    for(const auto & plane : hitOnPlaneMap){
      for(const auto & hit : plane.second){
        if(plane.first.arm() == 1) h2hitMapArm1->Fill(hit.globalPoint.x(),hit.globalPoint.y());
        if(plane.first.arm() == 0) h2hitMapArm0->Fill(hit.globalPoint.x(),hit.globalPoint.y());
      }
    }
